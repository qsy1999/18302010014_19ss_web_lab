...
function showWindowHref() {
    var url_string = url.value;
    var found = false;
    if (url_string.indexOf("?") !== -1) {
        var str = url_string.substr(url_string.indexOf("?") + 1);
        strs = str.split("&");
        for (var i = 0; i < strs.length; i++) {
            substrs = strs[i].split("=");
            if (substrs[0] === 'name') {
                url_result.value = substrs[1];
                found = true;
                break;
            }
        }
...
function arrSameStr() {
    var str = most.value;
    var characters = new Array(256);
    for (var i = 0; i < characters.length; i++) characters[i] = 0;
    for (var i = 0; i < str.length; i++) {
        characters[str.charCodeAt(i)]++;
    }
    var max = 0;
    var max_num = 0;
    ...
    result.value = String.fromCharCode(max_num) ;
}
